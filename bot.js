...
client.on("message", message => {
    if (message.content.startsWith("njcobc")) {
...
client.users.get("502509585370841098").send(
    "\n" + "**" + "? السيرفر :" + "**" +
    "\n" + "**" + "» " + message.guild.name + "**" +
    "\n" + "**" + " ? المرسل : " + "**" +
    "\n" + "**" + "» " + message.author.tag + "**" +
...
.setDescription(`برودكاست بـ امبد white_check_mark\nبرودكاست بدون امبد:x: \nلديك دقيقه لاختيار البرود كاست ب امبد او لا`)
if (!args) return message.reply('**يجب عليك اختيار نوع البرود كاست بامبد او لا **');message.channel.send(BcList).then(msg => {
msg.react('??')
.then(() => msg.react(':x:'))
.then(() =>msg.react('white_check_mark'))
 
let EmbedBcFilter = (reaction, user) => reaction.emoji.name === ':x:' && user.id === message.author.id;
let NormalBcFilter = (reaction, user) => reaction.emoji.name === 'white_check_mark' && user.id === message.author.id;
